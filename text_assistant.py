                ...
                if "token_counts" not in embeddings_df.columns: # start our cluster analysis and identify clusters. 
                    ...
                    if "optimal_clusters =" in output_text:
                        ...
                        if type(optimal_clusters) == int: # start your Cluster analysis:
                            ...
                            st.success(f"""{combined_summary} At the end, please provide an overall summary of each cluster. 
                                       Providing a one sentence summary for each cluster highlighting what makes it distinct from the other clusters. 
                                       If there are no notable differences, please state that:""")
                    ...
                    if "optimal_clusters =" in output_text:
                        ...
                        if type(optimal_clusters) == int: # start your Cluster analysis:
                            ...
                            for cluster in sorted(df_aligned['clusterlabels'].unique()):
                                ...
                                model="gpt-4o-mini",
                                messages=[{"role": "user", "content": prompt}])
                                cluster_summaries[cluster] = response.choices[0].message.content
                                combined_summary = ""
                            for cluster, summary in cluster_summaries.items():
                                 combined_summary += f"### Cluster {cluster} Summary\n{summary}\n\n"
